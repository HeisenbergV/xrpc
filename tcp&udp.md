...
1. 运输层如何才能在一种数据会丢失，会损坏的网络层上面构建可靠传输
2. 控制运输层的传输速率以避免网络中的拥塞，拥塞控制的原理。
3. TCP的具体实现
...
## udp
udp其实就是做了`多路复用/多路分解`，`差错检查`。除此之外没任何额外功能，和网络层唯一区别就是这点功能。由[RFC 768](https://tools.ietf.org/html/rfc768)定义的UDP只做运输层能够做的最少工作。
